...
import app from '../app';
/**
 *  maybe we can add DKIM support to prevent mails going to spam @abir-ban?
...
export default class Mailer {
  ...
  public async fanOutMail(recipients: string[], subject: string, templ: string, plaintext?: string): Promise<void> {
    try {
      const promises = recipients.map(async (recipient) => {
        const info = await this._transporter.sendMail({
          from: '"p-society" <psocietyiiitbh@gmail.com>',
          to: recipient,
          subject: subject,
          html: templ,
